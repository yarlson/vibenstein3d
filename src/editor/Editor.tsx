...
const Editor: React.FC = () => {
  ...
  const handleLoadLevel = useCallback(async (path: string) => {
    setIsLoading(true);
    setLoadError(null);
  ...
  const handleUpdateProperty = (position: GridPosition, property: string, value: number | string) => {
  ...
  if (loadError) {
    return (
      <div className="editor-error">
        ...
        <p>{loadError}</p>
        <button onClick={() => handleLoadLevel('/src/levels/level1.json')}>
          Try Again
        </button>
