...
export const useEditorStore = create<EditorState>((set, get) => ({
  ...
  updateGridDimensions: (rows: number, cols: number, cellSize: number) => {
    ...
    if (rows !== get().gridRows || cols !== get().gridCols) {
      ...
      const newGrid = Array(rows).fill(0).map((_, rowIndex) => {
      ...
      const newLights = Array(rows).fill(0).map((_, rowIndex) => {
        return Array(cols).fill(0).map((_, colIndex) => {
  ...
  updateLight: (position: GridPosition, lightType: number) => {
    ...
    const lights = levelData.lights || 
      Array(levelData.grid.length).fill(0).map(() => Array(levelData.grid[0].length).fill(0));
